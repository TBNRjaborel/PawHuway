...
function RneTab({ onCountUpdate }) {
    ...
    const groupedPast = groupNotificationsByDate(sortedPast);

    React.useEffect(() => {
        onCountUpdate(sortedPast.length);
    }, [sortedPast.length]);
            ...
            <TabView value={index} onChange={setIndex} animationType="spring">
                <TabView.Item style={{ width: '100%' }}>
                    <View style={{ paddingHorizontal: 18, paddingTop: 28 }}>
                        ...
                        {!loading && !error && sortedPast.length === 0 && (
                            <Text style={{ textAlign: 'center', marginTop: 20, color: '#555' }}>
                                No past tasks.
                            </Text>
                        )}
                        {!loading && !error &&
                            Object.entries(groupedPast).map(([dateLabel, items]) => (
...
export default function BacklogsScreen() {
                ...
                <Card style={styles.titleCard}>
                    <View style={{ flexDirection: 'row', marginLeft: 7, alignItems: 'center' }}>
                        ...
                        <View>
                            ...
                            <Text style={{ fontSize: 12 }}>{taskCount} tasks not addressed</Text>
                        </View>
                    </View>
                </Card>
            </View>
